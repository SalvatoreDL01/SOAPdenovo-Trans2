...
PUBLIC_FUNC KmerSet *init_kmerset (ubyte8 init_size, float load_factor)
{
	...
	/* Validate the load factor before computing max.  The old code
	 * computed max with the caller's value and only then sanitized it. */
	if (load_factor <= 0.0f)
	{
		load_factor = 0.25f;
	}
	...
	if (set == NULL)
	{
		fprintf (stderr, "-- Out of memory allocating KmerSet --\n");
		abort ();
	}
	...
	set->max = (ubyte8) (set->size * set->load_factor);
	set->iter_ptr = 0;
	flag_bytes = (size_t) ((set->size + 15) / 16 * 4);

	set->array = (kmer_t *) calloc (set->size, sizeof (kmer_t));
	set->flags = (ubyte4 *) malloc (flag_bytes);
	if (set->array == NULL || set->flags == NULL)
	{
		fprintf (stderr, "-- Out of memory allocating KmerSet arrays --\n");
		free (set->array);
		free (set->flags);
		free (set);
		abort ();
	}
	memset (set->flags, 0x55, flag_bytes);
	return set;
}
...
*/
	...
	{
		kmer_t *new_array = (kmer_t *) realloc (set->array, n * sizeof (kmer_t));
		if (new_array == NULL)
		{
			fprintf (stderr, "-- Out of memory growing KmerSet --\n");
			abort ();
		}
		set->array = new_array;
	}

	flags = (ubyte4 *) malloc ((size_t) ((n + 15) / 16 * 4));
	if (flags == NULL)
	{
		fprintf (stderr, "-- Out of memory growing KmerSet flags --\n");
		abort ();
	}
	memset (flags, 0x55, (size_t) ((n + 15) / 16 * 4));
	size = set->size;
	set->size = n;
