...
/*
 * Keep the k-mer staging buffers bounded. 100M entries can consume several
 * gigabytes before the first read batch is processed. The value can be
 * overridden with SOAP_KMER_BUFFER_SIZE when more throughput is desired.
 */
#define DEFAULT_KMER_BUFFER_SIZE 8000000U
static int buffer_size = DEFAULT_KMER_BUFFER_SIZE;

static void configure_kmer_buffer_size (void)
{
	const char *env = getenv ("SOAP_KMER_BUFFER_SIZE");
	if (env != NULL && *env != '\0')
	{
		unsigned long value = strtoul (env, NULL, 10);
		if (value >= 1024 && value <= 1000000000UL)
		{
...
boolean prlRead2HashTable (char *libfile, char *outfile)
{
	...
	if (1)
	{
		...
		/* Treat -a as a total initial budget and split it between shards.
		 * Without -a, size each shard from the current batch capacity so the
		 * default no longer starts at 1024 and repeatedly rehashes. */
		ubyte8 init_size = (initKmerSetSize > 0)
			? (ubyte8) initKmerSetSize / (thrd_num > 0 ? thrd_num : 1)
			: (ubyte8) buffer_size / (thrd_num > 0 ? thrd_num : 1);
		if (init_size < 1024)
		{
			init_size = 1024;
		}
		...
		for (i = 0; i < thrd_num; i++)
		{
			KmerSets[i] = init_kmerset (init_size, K_LOAD_FACTOR);
			thrdSignal[i + 1] = 0;
			paras[i].threadID = i;
